...
def __main__():
    ...
    with open("oierdb-data/static/contests.json") as f:
        for contest in json.load(f):
            Contest.create(contest)

    with open("oierdb-data/static/grades.json") as f:
        g_ = json.load(f)
    g_initial = g_["initial"]
    g_element = g_["element"]
    g_special = g_["special"]

    with open("oierdb-data/static/surnames.json") as f:
        surnames = json.load(f)

    with open("oierdb-data/static/scoring.json") as f:
        scoring = json.load(f)

    with open("oierdb-data/static/name_exceptions.json") as f:
