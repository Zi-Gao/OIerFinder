...
def create_app(config=None):
    ...
    # 全局变量存储数据文件路径
    DATA_FILE = config.get('data_file', 'data/serialized_data.pkl') if config else 'data/serialized_data.pkl'
    ...
    def find_oiers():
        ...
        # GET请求处理
            ...
            # 如果有yaml参数，将其设置为默认值
            default_yaml = yaml_param if yaml_param else ''
            
            return render_template('find_oiers.html', yaml_content=default_yaml)
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='OIer 查询系统')
    ...
    parser.add_argument('--debug', action='store_true', 
                        help='启用调试模式')
    parser.add_argument('--data-file', default='data/serialized_data.pkl',
                        help='数据文件路径 (默认: data/serialized_data.pkl)')
